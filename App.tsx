...
const App: React.FC = () => {
  ...
  const handleAddTransaction = (newTr: Transaction) => {
    ...
    if (newTr.type === 'OUT' || newTr.type === 'SHIFT') {
      const sourceItem = items.find(i => (i.id === newTr.itemId || i.name === newTr.itemName) && i.location === sourceLoc);
      if (!sourceItem || sourceItem.expectedQty < newTr.quantity) {
        showNotif('error', `Stok tidak mencukupi di ${sourceLoc}.`);
        return;
      }
    }
    ...
    setItems(prevItems => {
      ...
      const sourceIdx = nextItems.findIndex(i => i.name === itemDef.name && i.size === itemDef.size && i.location === sourceLoc);
    ...
    >
      ...
      {['mainWarehouse', 'singlesWarehouse', 'nuggetWarehouse', 'repairView', 'destructionView'].includes(activeTab) && (
        <WarehouseView 
          ...
          targetLocation={activeTab.includes('main') ? 'Gudang Utama' : activeTab.includes('singles') ? 'Gudang Singles' : activeTab.includes('nugget') ? 'Gudang Nugget' : activeTab.includes('repair') ? 'Repair' : 'Pemusnahan'}
