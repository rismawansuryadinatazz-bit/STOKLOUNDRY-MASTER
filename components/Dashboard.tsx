...
import { getDiscrepancySummary } from '../services/geminiService';
...
const Dashboard: React.FC<DashboardProps> = ({ items, theme, t, userName }) => {
  const [aiSummary, setAiSummary] = useState<string>('Analyzing discrepancies...');
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      setLoadingAi(true);
      const summary = await getDiscrepancySummary(items);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className={`lg:col-span-2 ${cardClass}`}>
          <h3 className={`text-lg font-bold mb-8 flex items-center ${textPrimary}`}>
            ...
            Stock Discrepancy Analysis (Top 10)
          </h3>
          <div className="h-80">
        ...
        <div className="space-y-8">
          <div className="bg-indigo-600 p-8 rounded-3xl shadow-2xl shadow-indigo-500/20 text-white relative overflow-hidden group">
            <div className="absolute top-0 right-0 -m-8 w-40 h-40 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all duration-700"></div>
            <h3 className="text-xl font-bold mb-6 flex items-center relative z-10">
              <i className="fas fa-robot mr-3 animate-pulse"></i>
              {t('aiInsight')}
            </h3>
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 min-h-[140px] border border-white/20 relative z-10">
              {loadingAi ? (
                <div className="flex flex-col items-center justify-center h-24 space-y-4">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div>
                  <p className="text-xs font-medium opacity-80 italic">Gemini is auditing...</p>
                </div>
              ) : (
                <p className="text-xs leading-relaxed whitespace-pre-line italic font-medium">
                  "{aiSummary}"
                </p>
              )}
            </div>
